...
export default ({ schedule }: { schedule: Schedule }) => (
    <>
        ...
        <div>
            <p className={`text-sm font-mono sm:hidden ${schedule.isActive ? 'text-green-500' : 'text-neutral-500'}`}>
                {schedule.isActive ? '[ Active ]' : '[ Inactive ]'}
            </p>
        </div>
        <ScheduleCronRow cron={schedule.cron} css={tw`mx-auto sm:mx-8 w-full sm:w-auto mt-4 sm:mt-0`} />
        <div>
            <p className={`text-sm font-mono hidden sm:block ${schedule.isProcessing ? 'text-blue-500' : schedule.isActive ? 'text-green-500' : 'text-neutral-500'}`}>
